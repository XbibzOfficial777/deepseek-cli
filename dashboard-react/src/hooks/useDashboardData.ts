...
import type { ApiActionRequest, ApiAdminNotifyRequest, ApiUserNotificationDeleteRequest } from '../lib/types';
...
export function useSendNotification() {
  return useMutation({
    mutationFn: (req: ApiAdminNotifyRequest) => api.sendNotification(req),
  });
}
...
export function useDeleteNotification() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (req: ApiUserNotificationDeleteRequest) => api.deleteNotification(req),
    onSuccess: () => {
