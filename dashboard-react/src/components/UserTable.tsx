import { Gavel, ShieldCheck, Edit3, Trash2, ShieldAlert, Send } from 'lucide-react';
import type { UserRecord } from '../lib/types';
import { formatNumber, formatNumberShort, getRelativeTime } from '../lib/format';

// Telegram support — shown for banned/limited rows
      ...
      <div className="table-wrap">
        <div className="empty-state">
          ...
          <p style={{ marginTop: 14, fontSize: 12, color: 'var(--text-muted)' }}>
            Need to appeal a ban or request a limit increase?{' '}
            <a
              href={TELEGRAM_URL}
              target="_blank"
            ...
            >
              Contact @XbibzOfficial on Telegram →
            </a>
          </p>
        </div>
      </div>
        ...
        <div className="user-cell">
          ...
          <div className="user-info">
            ...
            <div className="user-meta">
              ...
              <span className="dot" />
              v{user.version || '?'}
              {(user.banned || (limit > 0 && total >= limit)) && (
                <>
                  <span className="dot" />
                  <a
                    href={TELEGRAM_URL}
                    target="_blank"
                    rel="noopener noreferrer"
                    title="Appeal / request limit increase"
                    onClick={(e) => e.stopPropagation()}
                    style={{ color: 'var(--primary)', fontWeight: 600, fontSize: 10 }}
                  >
                    <Send size={9} style={{ verticalAlign: 'middle', marginRight: 2 }} />
                    telegram
                  </a>
                </>
              )}
