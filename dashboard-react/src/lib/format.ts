...
/**
 * Deterministic 8-char hex ID derived from a string. Used for the
 * admin session chip — never exposes the raw passcode.
 */
...
/**
 * Friendly display name for the admin session. Returns a short, readable
 * "admin_<short_id>" tag suitable for showing in the header chip.
 */
export function getAdminDisplayName(passcode: string): string {
  const id = computeAdminIdentifier(passcode);
  if (!id) return 'Admin';
  // First 4 hex chars as the human-readable handle, all-caps for clarity.
  return `Admin · ${id.slice(0, 4)}`;
}

export function getAdminAvatarSeed(passcode: string): string {
  return computeAdminIdentifier(passcode) || 'admin';
}

export function getAdminAvatarUrl(passcode: string): string {
  const seed = getAdminAvatarSeed(passcode);
